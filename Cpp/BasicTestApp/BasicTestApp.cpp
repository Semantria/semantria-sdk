...
BasicTestApp::BasicTestApp(std::string key, std::string secret) {
...
#ifdef _WIN32
    #include <windows.h>
    void BasicTestApp::sleep(unsigned milliseconds) {
        Sleep(milliseconds);
    }
#else
    #include <unistd.h>
...
void BasicTestApp::run() {
    cout << "Semantria Basic mode demo ..." << endl;
    queueDocuments();
...
void BasicTestApp::queueDocuments() {
    for(vector<string>::size_type i = 0; i != initialText->size(); i++) {
        ...
        if (202 >= session->QueueDocument(new Document(uid, initialText->at(i)))) {
            cout << "Document " << uid << " queued succsessfully." << endl;
        } else {
...
void BasicTestApp::receiveAnalyticData() {
...
void BasicTestApp::outputResults() {
    // Output results
...
string BasicTestApp::intToStr(int value) {
    std::stringstream stream;
    stream << value;
    return stream.str();
}

const int BasicTestApp::TIMEOUT_BEFORE_GETTING_RESPONSE = 5000; //in milliseconds
