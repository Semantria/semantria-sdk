...
AuthRequest::AuthRequest(string consumerKey, string consumerSecret, string configId, string applicationName, bool useCompression ) {
    init(consumerKey, consumerSecret, configId, applicationName, useCompression);
}

void AuthRequest::init(string consumerKey, string consumerSecret, string configId, string applicationName, bool useCompression) {
...
int AuthRequest::authWebRequest(string url, QueryMethod method, string body) {
    ...
    string timestamp = intToStr((unsigned int)time(NULL));
    ...
    if (( method == POST || method == DELETE )&& 0 != inputBuffer.length()) {
