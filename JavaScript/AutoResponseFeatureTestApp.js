;(function (){
	...
	window.runTestApp = function() {
		...
		log("<h1>Semantria Auto-response feature mode demo.</h1>");
		...
		SemantriaActiveSession.override({
			onError: function() {
				...
				log("Error: " + arguments[0].status + " " + arguments[0].message);
				errorFlag = true;
			},
			...
			onAfterResponse: function(result) {
				console.log("onAfterResponse:");
				console.log(arguments);
				if (result && result.length) {
					console.log("onAfterResponse recevied " + result.length + " documents");
		...
		var configurations = SemantriaActiveSession.getConfigurations(),
			oldPrimaryConfig = null,
			autoResponseTestConfig = null;
		
		for (var i in configurations) {
			// Remembers primary configuration to set it back after the test.
			if (configurations[i].is_primary) {
				oldPrimaryConfig = configurations[i];
			}
			
			if (configurations[i].name == 'AutoResponseTest') {
				autoResponseTestConfig = configurations[i];
			}
		}
		
		// Updates or Creates new configuration for the test purposes.
		log("Updates or Creates new configuration for the test purposes.<br/>");
		if (autoResponseTestConfig) {
			autoResponseTestConfig.is_primary = true;
			autoResponseTestConfig.auto_response = true;
			SemantriaActiveSession.updateConfigurations([autoResponseTestConfig]);
		} else {
			SemantriaActiveSession.addConfigurations([{
		...
		log("Queues documents for analysis one by one.<br><br>");
		if (outgoingDocs.length > 0) {
			var totalDocsCount = outgoingDocs.length,
				queuedDocsCount = 0;
			
			window.semantria_timeout = setInterval(function() {
				if (errorFlag) {
					clearInterval(window.semantria_timeout);
				}
				
				SemantriaActiveSession.queueDocument(outgoingDocs[i]);
				queuedDocsCount++;
				log("Documents queued/received rate: " + queuedDocsCount + "/" + docResultList.length + "<br/>");
				if (queuedDocsCount >= totalDocsCount) {
					clearInterval(window.semantria_timeout);
					
					if (totalDocsCount > docResultList.length) {
						// The final call to get remained data from server, Just for demo purposes.
						window.semantria_timeout = setInterval(function() {
							var docs = SemantriaActiveSession.getProcessedDocuments();
							if (docs && docs.length) {
								totalReceivedDocs += result.length;
								docResultList = docResultList.concat(docs);
								log("Documents queued/received rate: " + queuedDocsCount + "/" + docResultList.length + "s<br/>");
							}
							
							if (totalDocsCount <= docResultList.length) {
								// Return old primary configuration 
								oldPrimaryConfig.is_primary = true;
								SemantriaActiveSession.updateConfigurations([oldPrimaryConfig]);
